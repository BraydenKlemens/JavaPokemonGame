...
public class GameScreen implements Screen, InputProcessor {
	...
	private Sound click, door, walls;
	...
	public void show() {
		...
		walls = Gdx.audio.newSound(Gdx.files.internal("Music/wall.wav"));
					...
					if (Intersector.overlaps(playerRect, triggerRect)) {
						...
						if (object.getName().equalsIgnoreCase("pokeCenter")) {
							...
							if (foundPokemon()) {
								...
								game.setScreen(new BattleScreen(game, player.getPokemon().get(0), pokemon));
							}
					...
					if (Intersector.overlaps(playerRect, triggerRect)) {
						if (object.getName().equalsIgnoreCase("speach")) {
							...
							if (words.isEmpty() && Gdx.input.isKeyPressed(Input.Keys.SPACE) && location.equals("home")
								...
								if (!fight) {
									game.setScreen(
											new BattleScreen(game, player.getPokemon().get(0), misty.getPokemon()));
									fight = true;
								}
								if (fight) {
	...
	public boolean foundPokemon() {
		// find random pokemon 3/2000 chance
		int random = (int) (Math.random() * 2000);
		if (random == 10 || random == 500 || random == 1500) {
			return true;
		}
		return false;
	}
