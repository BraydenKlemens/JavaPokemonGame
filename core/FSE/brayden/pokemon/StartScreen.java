...
import PlantsvsZombies.OverLord;
...
public class StartScreen implements Screen, InputProcessor {
	...
	public void render(float delta) {
		...
		if (Gdx.input.isKeyPressed(Input.Keys.SPACE)) {
			music.stop();
			draw = true;
		} else if (Gdx.input.isKeyPressed(Input.Keys.B) && words.isEmpty() && text.isEmpty()) {
		...
		if (!draw) {
			...
			comm.draw(game.batch, "Click SPACE", 300, 100);
		} else {
			oak.draw(game.batch);
	...
	public void addText() {
		lockPrint = true;
		word.add("Press space to continue!");
